...
export default function ViewScore() {
  ...
  const handleSearch = async (idValue) => {
    ...
    try {
      ...
      const listIppt3 = (rows3 || []).filter(r => (r.sessions?.status === 'active')).map(r => ({
        ...
        run_2400_min: r.run_2400,
        school_type: r.sessions?.schools?.type || null,
        assessment_type: 'IPPT3',
      }))

      const merged = [...listNapfa, ...listIppt3]
      merged.sort((a,b) => {
  ...
  const resIppt3 = useMemo(() => {
    ...
    /* if (selectedIsIppt3 && details?.kind === 'IPPT3') {
    const pts = details.res3?.totalPoints || 0
    const award = details.res3?.award || 'No Award'
    return (
      <div className=\"p-4 bg-white/80 border rounded shadow-sm mb-4\">
        <div className=\"font-medium\">IPPT-3 Summary</div>
        <div className=\"mt-2 grid grid-cols-1 md:grid-cols-3 gap-3 text-sm\">
          <div><div className=\"text-gray-600\">Sit-ups</div><div className=\"font-mono\">{selected.situps ?? '-'}</div></div>
          <div><div className=\"text-gray-600\">Push-ups</div><div className=\"font-mono\">{selected.pushups ?? '-'}</div></div>
        ...
        <div className=\"mt-3 p-2 rounded border bg-slate-50 text-slate-800\">
          <div className=\"font-medium\">Total Points: {Number(pts)}</div>
          <div className=\"mt-1\">Award: {award}</div>
        </div>
      </div>
    )
  */
                ...
                <div className="space-y-3">
                  <div className="bg-white rounded shadow p-3">
                    ...
                    <div className="mt-3 p-2 rounded border bg-slate-50 text-slate-800">
                      ...
                      <div className="mt-1">Award: {resIppt3?.award || '-'}</div>
                    </div>
                  </div>
                </div>
              ) : details && (
...
function formatNextChip(key, next, unit) {
  if (!next) return '-'
  if (key === 'run') return next.target_mmss ? `ÃƒÂ¢Ã¢â‚¬Â°Ã‚Â¤ ${next.target_mmss}` : '-'
  if (key === 'shuttle_s') return Number.isFinite(next.target) ? `ÃƒÂ¢Ã¢â‚¬Â°Ã‚Â¤ ${Number(next.target).toFixed(1)} s` : '-'
  return next.target != null ? `ÃƒÂ¢Ã¢â‚¬Â°Ã‚Â¥ ${next.target}${unit ? ' ' + unit : ''}` : '-'
}
